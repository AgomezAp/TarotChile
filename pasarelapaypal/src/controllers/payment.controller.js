...
export const captureOrder = async (req, res) => {
  ...
  try {
    ...
    if (response.data.status === "COMPLETED") {
      ...
      return res.redirect(
        `https://cartastarotchile.com/descripcion-cartas?status=COMPLETED&token=${approvalToken}`
      );
    } else {
      ...
      return res.redirect(
        `https://cartastarotchile.com/descripcion-cartas?status=NOT_COMPLETED&token=${rejectToken}`
      );
    }
  } catch (error) {
    if (error.response) {
      console.error('Error de PayPal:', error.response.data);
      return res.redirect(`https://cartastarotchile.com/descripcion-cartas?status=ERROR`);
    }
